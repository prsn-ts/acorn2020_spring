...
public class JSONTestController {
	...
	@RequestMapping(value = "/json02", method=RequestMethod.GET,
			produces = "application/json; charset=utf-8") //클라이언트에게 어떤 컨텐츠(html 형식 or json 등등)를 보여줄 것인지 나타냄.
	@ResponseBody //리턴하는 문자열을 클라이언트에게 출력하겠다는 의미
	public String json02() {
		return "{\"num\":1,\"name\":\"kimgura\"}"; //jsp페이지로 이동시키는 것 뿐만 아니라 이렇게하면 응답할 문자열을 바로 보내줄 수 있다.
	}
	/*
	...
	//jackson-databind 라이브러리가 있어서 MemberDto 객체의 값도 출력이 가능하다.
