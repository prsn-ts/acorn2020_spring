 ...
 *  - @Component("bean 의 이름 지정") 어노테이션을 붙이므로써 Component scan에 의해 이름이 있는 bean이 된다.
 *  - servlet-context.xml 에 BeanNameViewResolver 설정을 추가한다.
 */
@Component("fileDownView") // "fileDownView"는 container에 들어간 bean의 이름이고 다른 bean 이름이랑 겹치면 안된다.
public class FileDownView extends AbstractView{
	
	//이 메소드에서 클라이언트에게 응답하면 된다.
	@Override
	protected void renderMergedOutputModel(Map<String, Object> model, HttpServletRequest request,
			HttpServletResponse response) throws Exception { //mView.addObject()에 담은 데이터는 Map 객체인 model에 담긴다.
		...
		String path=request.getServletContext().getRealPath("/upload") //getServletContext() 메소드로 application 영역에 저장된 값을 가져올 수 있다.
